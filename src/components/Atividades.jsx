...
import { Plus, RefreshCw, Pencil, Edit2, Trash2, X, CalendarDays, History, Paperclip, ArrowRight, Archive, RotateCcw, Search } from 'lucide-react'
...
export default function Atividades({profile}){
  ...
  const openTipo=(tipo,arquivo=false)=>{setTipoView(tipo);setArquivoView(arquivo);setOrdemView('recentes');setBuscaDetalhe('')}
...
{/* ── Minhas Rotinas ── */}
    ...
    {!tipoView? <div style={{display:'flex',flexDirection:'column',gap:28,marginTop:18}}><div style={{display:'flex',alignItems:'center',gap:8,background:C.white,border:'1px solid '+C.border,borderRadius:8,padding:'2px 12px',marginBottom:4}}><Search size={14} color={C.muted}/><input value={buscaGeral} onChange={e=>setBuscaGeral(e.target.value)} placeholder="Buscar em todas as atividades..." style={{border:'none',outline:'none',flex:1,padding:'9px 0',fontSize:14,color:C.text,background:'transparent'}}/>{buscaGeral&&<button onClick={()=>setBuscaGeral('')} style={{border:0,background:'none',cursor:'pointer',color:C.muted,display:'flex',padding:2}}><X size={13}/></button>}</div>{TIPOS.map(([tipo,nome])=>{const filtrados=buscaGeral?ativos.filter(i=>i.tipo===tipo&&matchBusca(i,buscaGeral)):grouped[tipo]||[];return <section key={tipo}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,paddingBottom:8,borderBottom:'2px solid '+C.border}}><h2 style={{margin:0,fontSize:16,fontWeight:800,color:C.text}}>{nome}{buscaGeral&&<span style={{fontSize:12,fontWeight:400,color:C.muted,marginLeft:8}}>({filtrados.length} resultado(s))</span>}</h2><div style={{display:'flex',gap:8,alignItems:'center'}}><button onClick={()=>openTipo(tipo,true)} style={{border:0,background:'none',color:C.green,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:4,fontSize:13}}><Archive size={13}/> arquivo ({archiveCounts[tipo]||0})</button><button onClick={()=>openTipo(tipo,false)} style={{border:0,background:'none',color:C.blue,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:4,fontSize:13}}>ver mais <ArrowRight size={13}/></button></div></div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:12}}>{filtrados.length?filtrados.map(renderCard):<div style={{padding:20,textAlign:'center',color:C.muted,border:'1px dashed '+C.border,borderRadius:10,gridColumn:'1/-1'}}>{buscaGeral?'Nenhum resultado para esta busca.':'Sem registros.'}</div>}</div></section>})}</div> : <div style={{marginTop:18}}><button onClick={()=>{setTipoView(null);setArquivoView(false)}} style={{...INP,width:'auto',marginBottom:12}}>← Voltar</button><div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',flexWrap:'wrap'}}><h2>{arquivoView?arquivoTitulo(tipoView):label(TIPOS,tipoView)}</h2><div style={{display:'flex',gap:8,alignItems:'center'}}><select value={ordemView} onChange={e=>setOrdemView(e.target.value)} style={{...INP,width:'auto',flexShrink:0}}><option value="recentes">Mais recentes</option><option value="antigas">Mais antigas</option><option value="data_asc">Por data do evento ↑</option><option value="data_desc">Por data do evento ↓</option></select><button onClick={()=>setArquivoView(v=>!v)} style={{border:'1px solid '+C.border,background:C.white,borderRadius:8,padding:'9px 12px',fontWeight:800,cursor:'pointer',display:'flex',gap:6,alignItems:'center'}}>{arquivoView?<><ArrowRight size={14}/> Ver ativos</>:<><Archive size={14}/> Ver arquivo</>}</button></div></div><div style={{display:'flex',alignItems:'center',gap:8,background:C.white,border:'1px solid '+C.border,borderRadius:8,padding:'2px 12px',margin:'12px 0'}}><Search size={14} color={C.muted}/><input value={buscaDetalhe} onChange={e=>setBuscaDetalhe(e.target.value)} placeholder={`Buscar em ${label(TIPOS,tipoView).toLowerCase()}...`} style={{border:'none',outline:'none',flex:1,padding:'9px 0',fontSize:14,color:C.text,background:'transparent'}}/>{buscaDetalhe&&<button onClick={()=>setBuscaDetalhe('')} style={{border:0,background:'none',cursor:'pointer',color:C.muted,display:'flex',padding:2}}><X size={13}/></button>}</div>{tipoView==='audiencia'&&!arquivoView&&<div style={{background:C.white,border:'1px solid '+C.border,borderRadius:12,padding:14,marginBottom:14}}><h3 style={{marginTop:0,display:'flex',gap:8,alignItems:'center'}}><CalendarDays size={16}/>Calendário de audiências</h3><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:8}}>{currentList.filter(a=>matchBusca(a,buscaDetalhe)&&a.prazo).slice().sort((a,b)=>String(a.prazo||'').localeCompare(String(b.prazo||''))).map(a=><div key={a.id} style={{border:'1px solid '+C.border,borderRadius:10,padding:10}}><b>{dateBR(a.prazo)}</b><div style={{fontSize:12,color:C.muted}}>{a.horario||'sem horário'} · {a.titulo}</div></div>)}{!currentList.filter(a=>matchBusca(a,buscaDetalhe)&&a.prazo).length&&<div style={{color:C.muted}}>Nenhuma audiência agendada.</div>}</div></div>}<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:12}}>{currentList.filter(a=>matchBusca(a,buscaDetalhe)).map(renderCard)}{!currentList.filter(a=>matchBusca(a,buscaDetalhe)).length&&<div style={{padding:22,color:C.muted,border:'1px dashed '+C.border,borderRadius:12}}>{buscaDetalhe?'Nenhum resultado para esta busca.':(arquivoView?'Nenhum registro arquivado neste tipo.':'Nenhum registro encontrado.')}</div>}</div></div>}
