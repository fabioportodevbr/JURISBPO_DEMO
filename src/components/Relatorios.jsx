...
function Card({title,value,sub,icon,color=C.navy,prv=false}){return <div style={{background:C.white,border:'1px solid '+C.border,borderRadius:14,padding:16}}><div style={{display:'flex',gap:8,alignItems:'center',fontSize:12,fontWeight:900,color:C.muted,textTransform:'uppercase'}}>{icon}{title}</div><div style={{fontSize:28,fontWeight:900,color,marginTop:12}} className={prv?'prv':''}>{value}</div>{sub&&<div style={{fontSize:12,color:C.muted}}>{sub}</div>}</div>}
