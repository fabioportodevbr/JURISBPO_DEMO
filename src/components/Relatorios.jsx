...
const C={navy:'#050505',white:'#fff',text:'#0f172a',muted:'#64748b',border:'#e5e7eb',green:'#16a34a',amber:'#b45309',red:'#dc2626',blue:'#1d4ed8',bg:'#f8fafc',soft:'#f1f5f9'}
...
function dataGastoCampo(f={},campo){if(campo==='seguro_premio')return dataPagamentoCampo(f,campo)||f.apolice_inicio||f.data_referencia||f.created_at;if(campo==='valor_bruto')return f.data_referencia||f.primeiro_vencimento||f.data_vencimento||f.created_at;return dataPagamentoCampo(f,campo)||f.data_referencia||f.primeiro_vencimento||f.data_vencimento||f.created_at}
const CAMPOS_ENCARGOS=['deposito_ro','deposito_rr','deposito_embargos','custas','fgts','honorarios_sucumbenciais','honorarios_periciais','inss_reclamante','inss_reclamada','multa_inadimplemento']
...
function totalFinanceiro(f){return itensFinanceiros(f).reduce((s,x)=>s+x.valor,0)-parseMoney(f.depositos_recursais_utilizados)}
...
export default function Relatorios({profile}){
  ...
  const st=useMemo(()=>{
    ...
    const baseCat=()=>({acordo:0,execucao:0,seguro:0,encargos:0,total:0})
  ...
  function formatValue(row,field){
    if(['valor_acao','valor_gasto','valor_economizado','valor_bruto','depositos_recursais_utilizados','custas','fgts','honorarios_sucumbenciais','honorarios_periciais','inss_reclamante','inss_reclamada','multa_inadimplemento','valor_assegurado','seguro_premio','total'].includes(field))return money(row[field])
  ...
  function gerarDados(){
    ...
    if(kind==='processos'){
      ...
      rows=rows.filter(p=>within(p.data_ajuizamento||p.updated_at||p.created_at,inicio,fim)).map(p=>({...p,__kind:'processo',valor_gasto:totalGastoDoProcesso(p,data.financeiros),valor_economizado:economiaReal(p,data.financeiros)}))
      ...
      rows=rows.filter(f=>itensFinanceiros(f).some(item=>within(item.data,inicio,fim))).map(f=>({...f,__kind:'financeiro',total:totalFinanceiro(f)}))
    } else {
  ...
  return <div style={{padding:24}}>
    {mode==='dashboard'?<>
      ...
      <section style={{background:C.white,border:'1px solid '+C.border,borderRadius:14,marginTop:20,overflow:'hidden'}}><h2 style={{fontSize:16,margin:0,padding:16,borderBottom:'1px solid '+C.border}}>Gastos do mês e natureza — {st.mesAtualLabel}</h2><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}><thead><tr style={{background:C.bg,color:C.muted,textAlign:'left'}}><th style={{padding:12}}>Acordo</th><th style={{padding:12}}>Execução</th><th style={{padding:12}}>Seguro-garantia</th><th style={{padding:12}}>Encargos</th><th style={{padding:12}}>Total do mês</th></tr></thead><tbody><tr style={{borderTop:'1px solid '+C.border}}><td style={{padding:12}}>{money(st.gastosMesAtual.acordo)}</td><td style={{padding:12}}>{money(st.gastosMesAtual.execucao)}</td><td style={{padding:12}}>{money(st.gastosMesAtual.seguro)}</td><td style={{padding:12}}>{money(st.gastosMesAtual.encargos)}</td><td style={{padding:12,fontWeight:900}}>{money(st.gastosMesAtual.total)}</td></tr></tbody></table></div></section>
      <section style={{background:C.white,border:'1px solid '+C.border,borderRadius:14,marginTop:20,overflow:'hidden'}}><h2 style={{fontSize:16,margin:0,padding:16,borderBottom:'1px solid '+C.border}}>Acumulados gerenciais</h2><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}><thead><tr style={{background:C.bg,color:C.muted,textAlign:'left'}}><th style={{padding:12}}>Período</th><th style={{padding:12}}>Acordo</th><th style={{padding:12}}>Execução</th><th style={{padding:12}}>Seguro-garantia</th><th style={{padding:12}}>Encargos</th><th style={{padding:12}}>Total</th></tr></thead><tbody>{[[`Ano atual ${st.anoAtual}`,st.gastosAnoAtual],[`Ano anterior ${st.anoAnterior}`,st.gastosAnoAnterior]].map(([periodo,v])=><tr key={periodo} style={{borderTop:'1px solid '+C.border}}><td style={{padding:12,fontWeight:800}}>{periodo}</td><td style={{padding:12}}>{money(v.acordo)}</td><td style={{padding:12}}>{money(v.execucao)}</td><td style={{padding:12}}>{money(v.seguro)}</td><td style={{padding:12}}>{money(v.encargos)}</td><td style={{padding:12,fontWeight:900}}>{money(v.total)}</td></tr>)}</tbody></table></div></section>
