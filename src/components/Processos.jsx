...
const REST_ORIGENS_MARK='[VALOR_RESTITUIDO_ORIGENS:'
const REST_ORIGENS=[['deposito_ro','Depósito de RO'],['deposito_rr','Depósito de RR'],['deposito_embargos','Depósito de embargos'],['apolice_seguro_garantia','Apólice seguro-garantia']]
...
function RestituicaoOrigensField({fin,setFin}){if(parseMoney(fin.valor_restituido)<=0)return null;const selecionadas=extrairOrigensRestituicao(fin);const toggle=(key)=>setFin(f=>{const atuais=extrairOrigensRestituicao(f);const nova=atuais.includes(key)?atuais.filter(x=>x!==key):[...atuais,key];return {...f,valor_restituido_origens:nova}});return <div style={{border:'1px solid '+C.border,borderRadius:10,padding:12,margin:'-4px 0 12px',background:C.grayBg}}><div style={{fontSize:12,fontWeight:900,color:C.text,marginBottom:8}}>Origem da restituição</div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}>{REST_ORIGENS.map(([key,label])=><label key={key} style={{display:'inline-flex',alignItems:'center',gap:6,fontSize:12,fontWeight:800,color:C.text,background:C.white,border:'1px solid '+(selecionadas.includes(key)?C.blue:C.border),borderRadius:999,padding:'7px 10px',cursor:'pointer'}}><input type="checkbox" checked={selecionadas.includes(key)} onChange={()=>toggle(key)} style={{accentColor:C.blue}}/>{label}</label>)}</div><div style={{fontSize:11,color:C.muted,marginTop:8}}>Obrigatório quando houver valor restituído.</div></div>}
...
function limparOrigensRestituicao(observacoes=''){return String(observacoes||'').replace(/\n?\[VALOR_RESTITUIDO_ORIGENS:[^\]]*\]/g,'').trim()}
...
function extrairOrigensRestituicao(f={}){
  const direto=Array.isArray(f.valor_restituido_origens)?f.valor_restituido_origens:[]
  if(direto.length)return direto.filter(k=>REST_ORIGENS.some(([key])=>key===k))
  const m=String(f.observacoes||'').match(/\[VALOR_RESTITUIDO_ORIGENS:([^\]]*)\]/)
  if(!m)return []
  try{
    const arr=JSON.parse(decodeURIComponent(m[1]))
    return Array.isArray(arr)?arr.filter(k=>REST_ORIGENS.some(([key])=>key===k)):[]
  }catch{return []}
}
function juntarOrigensRestituicao(observacoes='',origens=[]){
  const limpas=limparOrigensRestituicao(observacoes)
  const validas=(origens||[]).filter(k=>REST_ORIGENS.some(([key])=>key===k))
  const marcador=validas.length?`${REST_ORIGENS_MARK}${encodeURIComponent(JSON.stringify(validas))}]`:''
  return [limpas,marcador].filter(Boolean).join('\n')
}
function labelOrigensRestituicao(f={}){
  return extrairOrigensRestituicao(f).map(k=>REST_ORIGENS.find(([key])=>key===k)?.[1]||k).join(', ')
}
...
function provisionamentoProcesso(proc={}){return parseMoney(proc?.valor_acao)*0.035}
...
export default function Processos({profile}){
  ...
  const load=async()=>{const eid=profile.escritorio_id;const[p,{data:l},a,f,pc]=await Promise.all([fetchAllRows(()=>supabase.from('processos').select('*').eq('escritorio_id',eid).order('updated_at',{ascending:false})),supabase.from('usuarios_escritorios').select('usuario_id,papel,profiles(id,nome,email)').eq('escritorio_id',eid).eq('ativo',true),fetchAllRows(()=>supabase.from('atividades').select('*').eq('escritorio_id',eid).order('created_at',{ascending:false})),fetchAllRows(()=>supabase.from('financeiro_processos').select('*').eq('escritorio_id',eid).order('created_at',{ascending:false})),fetchAllRows(()=>supabase.from('partes_crm').select('*').eq('escritorio_id',eid).eq('status','ativo').order('nome'))]);setItems(p||[]);setTeam((l||[]).map(x=>({id:x.usuario_id,nome:x.profiles?.nome||x.profiles?.email||x.usuario_id})));setAtividades(a||[]);setFinanceiros(f||[]);setPartes(pc||[]);setLoading(false)}
  ...
  const hasValorRestituidoOrigensColumn=financeiros.some(f=>Object.prototype.hasOwnProperty.call(f,'valor_restituido_origens'))
  ...
  const persistFinance=async(finBase=fin,{manterFormulario=false,acao=null}={})=>{
    const valorRestituido=parseMoney(finBase.valor_restituido)
    const origensRestituicao=extrairOrigensRestituicao(finBase)
    if(valorRestituido>0&&!origensRestituicao.length){alert('Informe a origem do valor restituído: depósito de RO, depósito de RR, depósito de embargos ou apólice seguro-garantia.');return false}
    const auditBase=usuarioAuditoria(profile)
    const auditoriaAtual=[...(finBase.auditoria_pagamentos||[])]
    auditoriaAtual.push({acao:acao||(editingFinanceId?'edicao_registro_financeiro':'criacao_registro_financeiro'),...auditBase,registro_id:editingFinanceId||finBase.id||null})
    const qtd=finBase.forma_pagamento==='parcelado'?Math.max(2,Number(finBase.numero_parcelas||2)):1
    const dividir=finBase.forma_pagamento!=='parcelado'||finBase.dividir_valor_parcelas!==false
    const vencs=Array.from({length:qtd},(_,i)=>(finBase.vencimentos_parcelas?.[i]||(i===0?finBase.primeiro_vencimento:'')||''))
    const valores=dividir?[]:Array.from({length:qtd},(_,i)=>parseMoney(finBase.valores_parcelas?.[i]||0))
    const obsValor=hasValorRestituidoColumn?limparValorRestituido(finBase.observacoes):juntarValorRestituido(finBase.observacoes,finBase.valor_restituido)
    const obsBase=hasValorRestituidoOrigensColumn?limparOrigensRestituicao(obsValor):juntarOrigensRestituicao(obsValor,origensRestituicao)
    const obsParcelas=juntarObservacoesParcelas(obsBase,vencs,valores,dividir)
    const payload={...finBase,forma_pagamento:finBase.forma_pagamento==='parcelado'?'parcelado':'avista',escritorio_id:profile.escritorio_id,processo_id:form.id,criado_por:finBase.criado_por||profile.id,numero_parcelas:qtd,primeiro_vencimento:vencs[0]||null,observacoes:juntarAuditoriaPagamentos(juntarPagamentosCampos(obsParcelas,finBase.pagamentos_campos),auditoriaAtual),seguro_garantia:!!finBase.seguro_garantia}
    ;['id','created_at','updated_at','processos','vencimentos_parcelas','valores_parcelas','dividir_valor_parcelas','pagamentos_campos','auditoria_pagamentos'].forEach(k=>delete payload[k])
    if(!hasValorRestituidoColumn)delete payload.valor_restituido
    if(!hasValorRestituidoOrigensColumn)delete payload.valor_restituido_origens
    ;['valor_bruto','deposito_ro','deposito_rr','deposito_embargos','custas','fgts','honorarios_sucumbenciais','honorarios_periciais','inss_reclamante','inss_reclamada','multa_inadimplemento','valor_assegurado','seguro_premio'].forEach(k=>payload[k]=finBase[k]===''?0:parseMoney(finBase[k]))
    if(hasValorRestituidoColumn)payload.valor_restituido=valorRestituido
    if(hasValorRestituidoOrigensColumn)payload.valor_restituido_origens=valorRestituido>0?origensRestituicao:[]
    ;['data_referencia','apolice_inicio','apolice_fim'].forEach(k=>payload[k]=finBase[k]||null)
    if(!payload.seguro_garantia){payload.apolice_numero=null;payload.apolice_inicio=null;payload.apolice_fim=null;payload.valor_assegurado=0;payload.seguro_premio=0}
    const idAlvo=editingFinanceId||finBase.id
    const query=idAlvo?supabase.from('financeiro_processos').update(payload).eq('id',idAlvo).eq('escritorio_id',profile.escritorio_id):supabase.from('financeiro_processos').insert(payload)
    const {error}=await query
    if(error){alert(error.message);return false}
    if(!manterFormulario){setFin(emptyFinance);setEditingFinanceId(null)}
    await atualizarResumoFinanceiroProcesso(form.id)
    await load()
    return true
  }
  const saveFinance=async()=>{await persistFinance(fin,{manterFormulario:false})}
  const editFinance=(f)=>{setFin({...emptyFinance,...f,observacoes:limparOrigensRestituicao(limparValorRestituido(limparObservacoesParcelas(f.observacoes||''))),pagamentos_campos:extrairPagamentosCampos(f),auditoria_pagamentos:extrairAuditoriaPagamentos(f),vencimentos_parcelas:extrairVencimentosParcelas(f),dividir_valor_parcelas:extrairDividirParcelas(f),valores_parcelas:extrairValoresParcelas(f).map(v=>Number(v||0).toFixed(2)),numero_parcelas:f.numero_parcelas||1,seguro_garantia:!!f.seguro_garantia,valor_bruto:f.valor_bruto??'',deposito_ro:f.deposito_ro??'',deposito_rr:f.deposito_rr??'',deposito_embargos:f.deposito_embargos??'',custas:f.custas??'',fgts:f.fgts??'',honorarios_sucumbenciais:f.honorarios_sucumbenciais??'',honorarios_periciais:f.honorarios_periciais??'',inss_reclamante:f.inss_reclamante??'',inss_reclamada:f.inss_reclamada??'',multa_inadimplemento:f.multa_inadimplemento??'',valor_assegurado:f.valor_assegurado??'',seguro_premio:f.seguro_premio??'',valor_restituido:valorRestituidoFinanceiro(f)||'',valor_restituido_origens:extrairOrigensRestituicao(f)});setEditingFinanceId(f.id);setTab('financeiro');setTimeout(()=>document.getElementById('financeiro-form')?.scrollIntoView({behavior:'smooth',block:'start'}),50)}
  ...
  return <div style={{padding:24}}><AndamentosProcessuaisPush profile={profile} compact /><div style={{display:'flex',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}><div><h1 style={{margin:0,fontSize:22}}>Processos</h1><p style={{color:C.muted}}>{active.length} ativos · {closed.length} arquivados/encerrados</p></div>{can(profile,'processos.criar')&&<div style={{display:'flex',gap:8}}><button onClick={()=>{setImportModal(true);setImportRows([]);setImportLog([])}} style={{background:C.white,color:C.navy,border:'1px solid '+C.border,borderRadius:8,padding:'10px 16px',fontWeight:800,display:'flex',gap:8,alignItems:'center',cursor:'pointer'}}><Upload size={16}/>Importar CSV</button><button onClick={()=>open()} style={{background:C.navy,color:'white',border:0,borderRadius:8,padding:'10px 16px',fontWeight:800,display:'flex',gap:8,alignItems:'center'}}><Plus size={16}/>Novo Processo</button></div>}</div>
    ...
    <div style={{margin:'0 0 14px'}}><button onClick={()=>setShowAll(v=>!v)} style={{background:C.white,color:C.navy,border:'1px solid '+C.border,borderRadius:8,padding:'10px 16px',fontWeight:900,cursor:'pointer'}}>{showAll?'Ocultar todos':'Acessar todos'}</button></div>
    ...
    {modal&&<Modal title={form.id?'Processo completo':'Novo processo'} onClose={()=>setModal(false)}><div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}>{['dados','atividades','documentos','financeiro','andamentos'].map(t=><button key={t} onClick={()=>setTab(t)} disabled={!form.id&&t!=='dados'} style={{border:'1px solid '+C.border,borderRadius:20,padding:'7px 12px',background:tab===t?C.navy:C.white,color:tab===t?'white':C.text,cursor:'pointer',fontWeight:800,textTransform:'capitalize'}}>{t}</button>)}</div>
      ...
      {tab==='atividades'&&<><h3 style={{marginTop:0}}>Atividades do processo</h3><div style={{border:'1px solid '+C.border,borderRadius:12,padding:12,marginBottom:14,background:C.white}}>
        ...
        <div style={{display:'flex',justifyContent:'flex-end'}}><button onClick={saveTask} style={{background:C.navy,color:'white',border:0,borderRadius:8,padding:'11px 16px',fontWeight:800}}>Adicionar atividade</button></div>
      ...
      {tab==='financeiro'&&<>
        <h3 id="financeiro-form" style={{display:'flex',gap:8,alignItems:'center',marginTop:0}}><DollarSign size={18}/>Financeiro do processo</h3>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12}}>
          <F label="Provisionamento (3,5% do valor da ação)"><input style={{...INP,background:C.grayBg,fontWeight:900}} value={money(provisionamentoProcesso(form))} disabled title={`Valor da ação: ${money(form.valor_acao)}`}/></F>
          <F label="Natureza"><select style={INP} value={fin.natureza} onChange={e=>setFin({...fin,natureza:e.target.value})}>{NATUREZAS.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></F>
          <F label="Data do acordo/homologação"><input type="date" style={INP} value={fin.data_referencia} onChange={e=>setFin({...fin,data_referencia:e.target.value})}/></F>
          <F label="Valor bruto / total do acordo"><input type="text" inputMode="numeric" style={INP} value={moeda(fin.valor_bruto)} onChange={e=>moedaChange(e.target.value,v=>setFin(f=>({...f,valor_bruto:v})))} placeholder="R$ 0,00"/></F>
          <F label="Forma de pagamento"><select style={INP} value={fin.forma_pagamento} onChange={e=>{const v=e.target.value;setFin(f=>({...f,forma_pagamento:v,numero_parcelas:v==='avista'?1:Math.max(2,Number(f.numero_parcelas||2)),dividir_valor_parcelas:v==='avista'?true:f.dividir_valor_parcelas!==false,vencimentos_parcelas:v==='avista'?[f.vencimentos_parcelas?.[0]||f.primeiro_vencimento||'']:f.vencimentos_parcelas?.length?f.vencimentos_parcelas:['',''],valores_parcelas:v==='avista'?['']:f.valores_parcelas?.length?f.valores_parcelas:['','']}))}}>{FORMAS.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></F>
          <F label="Nº parcelas"><input type="number" min={fin.forma_pagamento==='parcelado'?2:1} style={INP} value={fin.forma_pagamento==='avista'?1:fin.numero_parcelas} disabled={fin.forma_pagamento==='avista'} onChange={e=>{const n=Math.max(2,Number(e.target.value||2));setFin(f=>({...f,numero_parcelas:n,vencimentos_parcelas:Array.from({length:n},(_,i)=>f.vencimentos_parcelas?.[i]||''),valores_parcelas:Array.from({length:n},(_,i)=>f.valores_parcelas?.[i]||'')}))}}/></F>
          {fin.forma_pagamento==='parcelado'&&<F label="Dividir valor total do acordo em parcelas iguais?"><select style={INP} value={fin.dividir_valor_parcelas===false?'nao':'sim'} onChange={e=>setFin(f=>({...f,dividir_valor_parcelas:e.target.value==='sim'}))}><option value="sim">Sim</option><option value="nao">Não</option></select></F>}
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12}}>
          {Array.from({length:fin.forma_pagamento==='parcelado'?Math.max(2,Number(fin.numero_parcelas||2)):1}).map((_,i)=><F key={i} label={`${ordinalParcela(i+1)} vencimento`}><input type="date" style={INP} value={fin.vencimentos_parcelas?.[i]||(i===0?fin.primeiro_vencimento:'')||''} onChange={e=>setFin(f=>{const arr=Array.from({length:f.forma_pagamento==='parcelado'?Math.max(2,Number(f.numero_parcelas||2)):1},(_,j)=>f.vencimentos_parcelas?.[j]||(j===0?f.primeiro_vencimento:'')||'');arr[i]=e.target.value;return {...f,vencimentos_parcelas:arr,primeiro_vencimento:arr[0]||''}})}/></F>)}
        </div>
        {fin.forma_pagamento==='parcelado'&&fin.dividir_valor_parcelas===false&&<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12}}>
          {Array.from({length:Math.max(2,Number(fin.numero_parcelas||2))}).map((_,i)=><F key={i} label={`Valor da parcela ${i+1}`}><input type="text" inputMode="numeric" style={INP} value={moeda(fin.valores_parcelas?.[i])} onChange={e=>moedaChange(e.target.value,v=>setFin(f=>{const arr=Array.from({length:Math.max(2,Number(f.numero_parcelas||2))},(_,j)=>f.valores_parcelas?.[j]||'');arr[i]=v;return {...f,valores_parcelas:arr}}))} placeholder="R$ 0,00"/></F>)}
        </div>}
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:12}}>
          <PaidValueField label="Depósito RO" field="deposito_ro" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
          <PaidValueField label="Depósito RR" field="deposito_rr" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
          <PaidValueField label="Depósito embargos" field="deposito_embargos" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:12}}>
          <F label="Valor restituído / devolvido"><input type="text" inputMode="numeric" style={INP} value={moeda(fin.valor_restituido)} onChange={e=>moedaChange(e.target.value,v=>setFin(f=>({...f,valor_restituido:v,valor_restituido_origens:parseMoney(v)>0?f.valor_restituido_origens:[]})))} placeholder="R$ 0,00"/></F>
        </div>
        <RestituicaoOrigensField fin={fin} setFin={setFin}/>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12}}>
          <PaidValueField label="Custas" field="custas" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
          <PaidValueField label="FGTS" field="fgts" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
          <PaidValueField label="Honorários sucumbenciais" field="honorarios_sucumbenciais" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
          <PaidValueField label="Honorários periciais" field="honorarios_periciais" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
          <PaidValueField label="INSS reclamante" field="inss_reclamante" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
          <PaidValueField label="INSS reclamada" field="inss_reclamada" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
          <PaidValueField label="Multa por inadimplemento" field="multa_inadimplemento" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
        </div>
        <div style={{border:'1px solid '+C.border,borderRadius:12,padding:12,margin:'12px 0'}}><b>Apólice / Seguro-garantia</b><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12,marginTop:10}}>
          <F label="Possui seguro-garantia?"><select style={INP} value={fin.seguro_garantia?'sim':'nao'} onChange={e=>setFin({...fin,seguro_garantia:e.target.value==='sim'})}><option value="nao">Não</option><option value="sim">Sim</option></select></F>
          <F label="Número da apólice"><input style={INP} value={fin.apolice_numero||''} onChange={e=>setFin({...fin,apolice_numero:e.target.value})}/></F>
          <F label="Início da vigência"><input type="date" style={INP} value={fin.apolice_inicio||''} onChange={e=>setFin({...fin,apolice_inicio:e.target.value})}/></F>
          <F label="Fim da vigência"><input type="date" style={INP} value={fin.apolice_fim||''} onChange={e=>setFin({...fin,apolice_fim:e.target.value})}/></F>
          <F label="Valor assegurado"><input type="text" inputMode="numeric" style={INP} value={moeda(fin.valor_assegurado)} onChange={e=>moedaChange(e.target.value,v=>setFin(f=>({...f,valor_assegurado:v})))} placeholder="R$ 0,00"/></F>
          <PaidValueField label="Prêmio pago" field="seguro_premio" fin={fin} setFin={setFin} onPaid={abrirPagamentoCampo}/>
        </div></div>
        <F label="Observações financeiras"><textarea style={{...INP,minHeight:70}} value={fin.observacoes} onChange={e=>setFin({...fin,observacoes:e.target.value})}/></F>
        <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
          <button onClick={saveFinance} style={{background:editingFinanceId?C.blue:C.navy,color:'white',border:0,borderRadius:8,padding:'11px 18px',fontWeight:800}}>{editingFinanceId?'Salvar alterações':'Adicionar pagamento/parcelamento'}</button>
          {editingFinanceId&&<button onClick={cancelEditFinance} style={{background:C.white,color:C.text,border:'1px solid '+C.border,borderRadius:8,padding:'11px 18px',fontWeight:800}}>Cancelar edição</button>}
          {editingFinanceId&&<span style={{fontSize:12,color:C.muted}}>Editando pagamento selecionado.</span>}
        </div>
        <h4 style={{marginTop:22}}>Pagamentos e parcelamentos registrados</h4>
        {procFin.map(f=><div key={f.id} style={{border:'1px solid '+C.border,borderRadius:10,padding:12,marginBottom:8}}><div style={{display:'flex',justifyContent:'space-between',gap:10,flexWrap:'wrap'}}><div><b>{tituloAPFinanceiro(f)} · total {money(totalAP(f))}</b><div style={{fontSize:12,color:C.muted}}>Referência: {dateBR(f.data_referencia)} · {label(FORMAS,f.forma_pagamento)} · {f.numero_parcelas||1} parcela(s) · valores: {extrairValoresParcelas(f).map((v,i)=>`${i+1}/${f.numero_parcelas||1}: ${money(v)}`).join(' · ')} · venc.: {extrairVencimentosParcelas(f).map((v,i)=>`${i+1}/${f.numero_parcelas||1}: ${dateBR(v)}`).join(' · ')}{(f.seguro_garantia||f.apolice_numero||Number(f.seguro_premio||0)>0)?` · Seguro-garantia: ${f.seguro_garantia?'sim':'não'} · Apólice ${f.apolice_numero||'sem número'} · vigência ${dateBR(f.apolice_inicio)} a ${dateBR(f.apolice_fim)} · assegurado ${money(f.valor_assegurado)} · prêmio ${money(f.seguro_premio)}`:''}{valorRestituidoFinanceiro(f)>0?` · Restituído ${money(valorRestituidoFinanceiro(f))}${labelOrigensRestituicao(f)?` · Origem: ${labelOrigensRestituicao(f)}`:''}`:''}</div></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{temPagamentoCampoAtivo(f)&&<span style={{fontSize:12,color:C.green,fontWeight:800,alignSelf:'center'}}>Pagamento marcado como pago</span>}<button onClick={()=>editFinance(f)} style={{border:'1px solid '+C.border,background:editingFinanceId===f.id?C.blueBg:C.white,color:editingFinanceId===f.id?C.blue:C.text,borderRadius:8,padding:'8px 10px',cursor:'pointer',fontWeight:800,display:'inline-flex',alignItems:'center',gap:6}}><Edit2 size={14}/> Editar</button>{temPagamentoCampoAtivo(f)&&<button onClick={()=>gerenciarPagamentoRegistro(f)} style={{border:'1px solid '+C.green,background:C.greenBg,color:C.green,borderRadius:8,padding:'8px 10px',cursor:'pointer',fontWeight:800,display:'inline-flex',alignItems:'center',gap:6}}>Gerenciar pagamento</button>}{!temPagamentoCampoAtivo(f)&&<button onClick={()=>gerarAutorizacao(f)} style={{border:'1px solid '+C.border,background:C.white,borderRadius:8,padding:'8px 10px',cursor:'pointer',fontWeight:800,display:'inline-flex',alignItems:'center',gap:6}}><Download size={14}/> Autorização</button>}{!temPagamentoCampoAtivo(f)&&<button onClick={()=>deleteFinance(f)} style={{border:'1px solid '+C.red,background:C.redBg,color:C.red,borderRadius:8,padding:'8px 10px',cursor:'pointer',fontWeight:800,display:'inline-flex',alignItems:'center',gap:6}}><Trash2 size={14}/> Excluir</button>}</div></div></div>)}
        {!procFin.length&&<div style={{color:C.muted,border:'1px dashed '+C.border,borderRadius:10,padding:18,marginTop:12}}>Nenhum pagamento registrado.</div>}
      </>}
