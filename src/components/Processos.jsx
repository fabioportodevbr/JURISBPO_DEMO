...
function totalAP(f){return totalFinanceiroRegistro(f)}
...
function totalFinanceiroRegistro(f){
...
function detalharGastosFinanceiros(financeiros=[]){
  ...
  return {linhas,total:linhas.reduce((s,x)=>s+x.valor,0)}
}
...
export default function Processos({profile}){
  ...
  const persistFinance=async(finBase=fin,{manterFormulario=false,acao=null}={})=>{const auditBase=usuarioAuditoria(profile);const auditoriaAtual=[...(finBase.auditoria_pagamentos||[])];auditoriaAtual.push({acao:acao||(editingFinanceId?'edicao_registro_financeiro':'criacao_registro_financeiro'),...auditBase,registro_id:editingFinanceId||finBase.id||null});const qtd=finBase.forma_pagamento==='parcelado'?Math.max(2,Number(finBase.numero_parcelas||2)):1;const dividir=finBase.forma_pagamento!=='parcelado'||finBase.dividir_valor_parcelas!==false;const vencs=Array.from({length:qtd},(_,i)=>(finBase.vencimentos_parcelas?.[i]||(i===0?finBase.primeiro_vencimento:'')||''));const valores=dividir?[]:Array.from({length:qtd},(_,i)=>parseMoney(finBase.valores_parcelas?.[i]||0));const payload={...finBase,forma_pagamento:finBase.forma_pagamento==='parcelado'?'parcelado':'avista',escritorio_id:profile.escritorio_id,processo_id:form.id,criado_por:finBase.criado_por||profile.id,numero_parcelas:qtd,primeiro_vencimento:vencs[0]||null,observacoes:juntarAuditoriaPagamentos(juntarPagamentosCampos(juntarObservacoesParcelas(finBase.observacoes,vencs,valores,dividir),finBase.pagamentos_campos),auditoriaAtual),seguro_garantia:!!finBase.seguro_garantia};['id','created_at','updated_at','processos','vencimentos_parcelas','valores_parcelas','dividir_valor_parcelas','pagamentos_campos','auditoria_pagamentos'].forEach(k=>delete payload[k]);['valor_bruto','deposito_ro','deposito_rr','deposito_embargos','custas','fgts','honorarios_sucumbenciais','honorarios_periciais','inss_reclamante','inss_reclamada','multa_inadimplemento','valor_assegurado','seguro_premio'].forEach(k=>payload[k]=finBase[k]===''?0:parseMoney(finBase[k]));['data_referencia','apolice_inicio','apolice_fim'].forEach(k=>payload[k]=finBase[k]||null);if(!payload.seguro_garantia){payload.apolice_numero=null;payload.apolice_inicio=null;payload.apolice_fim=null;payload.valor_assegurado=0;payload.seguro_premio=0}const idAlvo=editingFinanceId||finBase.id;const query=idAlvo?supabase.from('financeiro_processos').update(payload).eq('id',idAlvo).eq('escritorio_id',profile.escritorio_id):supabase.from('financeiro_processos').insert(payload);const {error}=await query;if(error){alert(error.message);return false}if(!manterFormulario){setFin(emptyFinance);setEditingFinanceId(null)}await atualizarResumoFinanceiroProcesso(form.id);await load();return true}
  const saveFinance=async()=>{await persistFinance(fin,{manterFormulario:false})}
  const editFinance=(f)=>{setFin({...emptyFinance,...f,observacoes:limparObservacoesParcelas(f.observacoes||''),pagamentos_campos:extrairPagamentosCampos(f),auditoria_pagamentos:extrairAuditoriaPagamentos(f),vencimentos_parcelas:extrairVencimentosParcelas(f),dividir_valor_parcelas:extrairDividirParcelas(f),valores_parcelas:extrairValoresParcelas(f).map(v=>Number(v||0).toFixed(2)),numero_parcelas:f.numero_parcelas||1,seguro_garantia:!!f.seguro_garantia,valor_bruto:f.valor_bruto??'',deposito_ro:f.deposito_ro??'',deposito_rr:f.deposito_rr??'',deposito_embargos:f.deposito_embargos??'',custas:f.custas??'',fgts:f.fgts??'',honorarios_sucumbenciais:f.honorarios_sucumbenciais??'',honorarios_periciais:f.honorarios_periciais??'',inss_reclamante:f.inss_reclamante??'',inss_reclamada:f.inss_reclamada??'',multa_inadimplemento:f.multa_inadimplemento??'',valor_assegurado:f.valor_assegurado??'',seguro_premio:f.seguro_premio??''});setEditingFinanceId(f.id);setTab('financeiro');setTimeout(()=>document.getElementById('financeiro-form')?.scrollIntoView({behavior:'smooth',block:'start'}),50)}
